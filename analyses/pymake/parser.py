...
from analyses.pymake import data, functions, util, parserdata, errors
    ...
    for d in fdlines:
        ...
        if m is not None:
            ...
            if kword == 'override':
                ...
                condstack[-1].append(
                    parserdata.SetVariable(vname, value=value, valueloc=d.getloc(offset), token=token, targetexp=None,
            ...
            if kword == 'export':
                ...
                if token is None:
                    ...
                    value = flattenmakesyntax(d, offset).lstrip()
                    condstack[-1].append(
                        parserdata.SetVariable(e, value=value, valueloc=d.getloc(offset), token=token, targetexp=None))
        ...
        if token in _varsettokens:
            ...
            if token in (None, ';'):
                ...
                value = flattenmakesyntax(d, offset).lstrip()
                condstack[-1].append(
                    parserdata.SetVariable(e, value=value, valueloc=d.getloc(offset), token=token, targetexp=targets))
            elif token == '|':
