...
static void qca_qca9558_init(MachineState *machine)
{
    ...
    set_autoboard_intc_cfg(AUTOBOARD_INTC_ATH79_GENERIC, "intc");
    ...
    sysbus_mmio_map(SYS_BUS_DEVICE(&s->autoboard_intc0), 0, 0x18060010);
    
    qdev_connect_gpio_out(DEVICE(&s->autoboard_intc0), 0, s->cpu->env.irq[6]);
    ...
    serial_mm_init(get_system_memory(), 0x18020000, 2, qdev_get_gpio_in(DEVICE(&s->autoboard_intc0), 3), 115200, serial_hd(0), DEVICE_BIG_ENDIAN);
