...
#define TYPE_MACH_BCM "mach_bcm"
#define MACH_BCM(obj) \
    OBJECT_CHECK(MACH_BCMState, (obj), TYPE_MACH_BCM)

typedef struct MACH_BCMState {
    ARMCPU *cpu;
...
static void mach_bcm_reset(void *opaque)
{
    MACH_BCMState *s = opaque;
    
}

static void mach_bcm_init(MachineState *machine)
{
    MACH_BCMState *s = g_new0(MACH_BCMState, 1);
    Error *err = NULL;
    ...
    sysbus_create_varargs("l2x0", (0x19021000 & 0xffff0000) + 0x2000, NULL);
    
    sysbus_connect_irq(SYS_BUS_DEVICE(&s->arm_cortex_a9_gic), 0, qdev_get_gpio_in(DEVICE(s->cpu), ARM_CPU_IRQ));
    
    
    serial_mm_init(get_system_memory(), 0x18000400, 0, qdev_get_gpio_in(DEVICE(&s->arm_cortex_a9_gic), 53), 115200, serial_hd(0), DEVICE_LITTLE_ENDIAN);
    serial_mm_init(get_system_memory(), 0x18000300, 0, qdev_get_gpio_in(DEVICE(&s->arm_cortex_a9_gic), 53), 115200, serial_hd(1), DEVICE_LITTLE_ENDIAN);
    
    memory_region_init_io(&s->stub0_mmio, NULL, &stub0_ops, s, TYPE_MACH_BCM, 0x300);
    memory_region_add_subregion_overlap(get_system_memory(), 0x18000000, &s->stub0_mmio, 0);
    memory_region_init_io(&s->stub2_mmio, NULL, &stub2_ops, s, TYPE_MACH_BCM, 0xb00);
    memory_region_add_subregion_overlap(get_system_memory(), 0x18000500, &s->stub2_mmio, 0);
    memory_region_init_io(&s->stub4_mmio, NULL, &stub4_ops, s, TYPE_MACH_BCM, 0x600);
    memory_region_add_subregion_overlap(get_system_memory(), 0x18028000, &s->stub4_mmio, 0);
    memory_region_init_io(&s->stub5_mmio, NULL, &stub5_ops, s, TYPE_MACH_BCM, 0x20);
    memory_region_add_subregion_overlap(get_system_memory(), 0x18028f00, &s->stub5_mmio, 0);
    memory_region_init_io(&s->stub6_mmio, NULL, &stub6_ops, s, TYPE_MACH_BCM, 0x600);
...
static void mach_bcm_machine_init(MachineClass *mc)
{
    mc->desc = "mach_bcm";
    mc->init = mach_bcm_init;
    mc->default_ram_size = 0x8000000;
    mc->default_cpu_type = ARM_CPU_TYPE_NAME("cortex-a9");
    mc->ignore_memory_transaction_failures = false;
}

DEFINE_MACHINE("mach_bcm", mach_bcm_machine_init)
