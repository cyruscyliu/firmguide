...
#define TYPE_MACH_KIRKWOOD "mach_kirkwood"
#define MACH_KIRKWOOD(obj) \
    OBJECT_CHECK(MACH_KIRKWOODState, (obj), TYPE_MACH_KIRKWOOD)

typedef struct MACH_KIRKWOODState {
    ARMCPU *cpu;
    ...
    MemoryRegion stub44_mmio;
    uint32_t stub_reserved44[0x800 >> 2];
    ...
    uint32_t stub_reserved43[0x100000 >> 2];
    MemoryRegion stub41_mmio;
    uint32_t stub_reserved41[0x400 >> 2];
    MemoryRegion stub42_mmio;
    uint32_t stub_reserved42[0x800 >> 2];
    MemoryRegion stub45_mmio;
    uint32_t stub_reserved45[0x1000 >> 2];
    MemoryRegion ram;
} MACH_KIRKWOODState;

static void stub0_update(void *opaque)
{
    /* MACH_KIRKWOODState *s = opaque; */
}
...
static void stub44_update(void *opaque)
{
    /* MACH_KIRKWOODState *s = opaque; */
}

static uint64_t stub44_read(void *opaque, hwaddr offset, unsigned size)
{
    MACH_KIRKWOODState *s = opaque;
    uint32_t res = 0;
    ...
    default:
        ...
        return 0;
    case 0x0 ... 0x7fc:
        res = s->stub_reserved44[offset >> 2];
        break;
    }
    return res;
}

static void stub44_write(void *opaque, hwaddr offset, uint64_t val, unsigned size)
{
    MACH_KIRKWOODState *s = opaque;
    ...
    default:
        ...
        return;
    case 0x0 ... 0x7fc:
        s->stub_reserved44[offset >> 2] = val;
        break;
    }
    stub44_update(s);
}

static const MemoryRegionOps stub44_ops = {
    .read = stub44_read,
    .write = stub44_write,
    .endianness = DEVICE_LITTLE_ENDIAN,
};
...
static void stub45_update(void *opaque)
{
    /* MACH_KIRKWOODState *s = opaque; */
}

static uint64_t stub45_read(void *opaque, hwaddr offset, unsigned size)
{
    MACH_KIRKWOODState *s = opaque;
    uint32_t res = 0;
    ...
    default:
        ...
        return 0;
    case 0x0 ... 0xffc:
        res = s->stub_reserved45[offset >> 2];
        break;
    }
    return res;
}

static void stub45_write(void *opaque, hwaddr offset, uint64_t val, unsigned size)
{
    MACH_KIRKWOODState *s = opaque;
    ...
    default:
        ...
        return;
    case 0x0 ... 0xffc:
        s->stub_reserved45[offset >> 2] = val;
        break;
    }
    stub45_update(s);
}

static const MemoryRegionOps stub45_ops = {
    .read = stub45_read,
    .write = stub45_write,
    .endianness = DEVICE_LITTLE_ENDIAN,
};

static void mach_kirkwood_reset(void *opaque)
{
    MACH_KIRKWOODState *s = opaque;
    
}

static void mach_kirkwood_init(MachineState *machine)
{
    MACH_KIRKWOODState *s = g_new0(MACH_KIRKWOODState, 1);
    Error *err = NULL;
    ...
    serial_mm_init(get_system_memory(), 0xf1012100, 2, qdev_get_gpio_in(DEVICE(&s->marvell_orion_intc0), 2), 115200, serial_hd(0), DEVICE_LITTLE_ENDIAN);
    serial_mm_init(get_system_memory(), 0xf1012000, 2, qdev_get_gpio_in(DEVICE(&s->marvell_orion_intc0), 1), 115200, serial_hd(1), DEVICE_LITTLE_ENDIAN);
    
    memory_region_init_io(&s->stub0_mmio, NULL, &stub0_ops, s, TYPE_MACH_KIRKWOOD, 0x20);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1001500, &s->stub0_mmio, 0);
    memory_region_init_io(&s->stub1_mmio, NULL, &stub1_ops, s, TYPE_MACH_KIRKWOOD, 0x20);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1010000, &s->stub1_mmio, 0);
    memory_region_init_io(&s->stub2_mmio, NULL, &stub2_ops, s, TYPE_MACH_KIRKWOOD, 0x4);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1010030, &s->stub2_mmio, 0);
    memory_region_init_io(&s->stub3_mmio, NULL, &stub3_ops, s, TYPE_MACH_KIRKWOOD, 0x4);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1010078, &s->stub3_mmio, 0);
    memory_region_init_io(&s->stub4_mmio, NULL, &stub4_ops, s, TYPE_MACH_KIRKWOOD, 0x40);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1010100, &s->stub4_mmio, 0);
    memory_region_init_io(&s->stub5_mmio, NULL, &stub5_ops, s, TYPE_MACH_KIRKWOOD, 0x40);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1010140, &s->stub5_mmio, 0);
    memory_region_init_io(&s->stub6_mmio, NULL, &stub6_ops, s, TYPE_MACH_KIRKWOOD, 0x20);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1010300, &s->stub6_mmio, 0);
    memory_region_init_io(&s->stub7_mmio, NULL, &stub7_ops, s, TYPE_MACH_KIRKWOOD, 0x28);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1010600, &s->stub7_mmio, 0);
    memory_region_init_io(&s->stub8_mmio, NULL, &stub8_ops, s, TYPE_MACH_KIRKWOOD, 0x20);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1011000, &s->stub8_mmio, 0);
    memory_region_init_io(&s->stub9_mmio, NULL, &stub9_ops, s, TYPE_MACH_KIRKWOOD, 0x20);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1011100, &s->stub9_mmio, 0);
    memory_region_init_io(&s->stub13_mmio, NULL, &stub13_ops, s, TYPE_MACH_KIRKWOOD, 0x80);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1020000, &s->stub13_mmio, 0);
    memory_region_init_io(&s->stub12_mmio, NULL, &stub12_ops, s, TYPE_MACH_KIRKWOOD, 0x88);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1020080, &s->stub12_mmio, 0);
    memory_region_init_io(&s->stub14_mmio, NULL, &stub14_ops, s, TYPE_MACH_KIRKWOOD, 0x4);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1020108, &s->stub14_mmio, 0);
    memory_region_init_io(&s->stub15_mmio, NULL, &stub15_ops, s, TYPE_MACH_KIRKWOOD, 0x4);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf102010c, &s->stub15_mmio, 0);
    memory_region_init_io(&s->stub17_mmio, NULL, &stub17_ops, s, TYPE_MACH_KIRKWOOD, 0x4);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1020118, &s->stub17_mmio, 0);
    memory_region_init_io(&s->stub18_mmio, NULL, &stub18_ops, s, TYPE_MACH_KIRKWOOD, 0x4);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf102011c, &s->stub18_mmio, 0);
    memory_region_init_io(&s->stub19_mmio, NULL, &stub19_ops, s, TYPE_MACH_KIRKWOOD, 0x4);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1020128, &s->stub19_mmio, 0);
    memory_region_init_io(&s->stub22_mmio, NULL, &stub22_ops, s, TYPE_MACH_KIRKWOOD, 0x8);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1020320, &s->stub22_mmio, 0);
    memory_region_init_io(&s->stub24_mmio, NULL, &stub24_ops, s, TYPE_MACH_KIRKWOOD, 0x10000);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1030000, &s->stub24_mmio, 0);
    memory_region_init_io(&s->stub44_mmio, NULL, &stub44_ops, s, TYPE_MACH_KIRKWOOD, 0x800);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1040000, &s->stub44_mmio, 0);
    memory_region_init_io(&s->stub25_mmio, NULL, &stub25_ops, s, TYPE_MACH_KIRKWOOD, 0x2000);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1040800, &s->stub25_mmio, 0);
    memory_region_init_io(&s->stub26_mmio, NULL, &stub26_ops, s, TYPE_MACH_KIRKWOOD, 0x2000);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1045000, &s->stub26_mmio, 0);
    memory_region_init_io(&s->stub27_mmio, NULL, &stub27_ops, s, TYPE_MACH_KIRKWOOD, 0x1000);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1050000, &s->stub27_mmio, 0);
    memory_region_init_io(&s->stub28_mmio, NULL, &stub28_ops, s, TYPE_MACH_KIRKWOOD, 0x100);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1060800, &s->stub28_mmio, 0);
    memory_region_init_io(&s->stub29_mmio, NULL, &stub29_ops, s, TYPE_MACH_KIRKWOOD, 0x100);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1060900, &s->stub29_mmio, 0);
    memory_region_init_io(&s->stub30_mmio, NULL, &stub30_ops, s, TYPE_MACH_KIRKWOOD, 0x100);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1060a00, &s->stub30_mmio, 0);
    memory_region_init_io(&s->stub31_mmio, NULL, &stub31_ops, s, TYPE_MACH_KIRKWOOD, 0x100);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1060b00, &s->stub31_mmio, 0);
    memory_region_init_io(&s->stub32_mmio, NULL, &stub32_ops, s, TYPE_MACH_KIRKWOOD, 0x4000);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1072000, &s->stub32_mmio, 0);
    memory_region_init_io(&s->stub33_mmio, NULL, &stub33_ops, s, TYPE_MACH_KIRKWOOD, 0x4000);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1076000, &s->stub33_mmio, 0);
    memory_region_init_io(&s->stub34_mmio, NULL, &stub34_ops, s, TYPE_MACH_KIRKWOOD, 0x2000);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1080000, &s->stub34_mmio, 0);
    memory_region_init_io(&s->stub35_mmio, NULL, &stub35_ops, s, TYPE_MACH_KIRKWOOD, 0x334);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1082000, &s->stub35_mmio, 0);
    memory_region_init_io(&s->stub36_mmio, NULL, &stub36_ops, s, TYPE_MACH_KIRKWOOD, 0x1ccc);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1082334, &s->stub36_mmio, 0);
    memory_region_init_io(&s->stub37_mmio, NULL, &stub37_ops, s, TYPE_MACH_KIRKWOOD, 0x334);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1084000, &s->stub37_mmio, 0);
    memory_region_init_io(&s->stub38_mmio, NULL, &stub38_ops, s, TYPE_MACH_KIRKWOOD, 0xccc);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1084334, &s->stub38_mmio, 0);
    memory_region_init_io(&s->stub39_mmio, NULL, &stub39_ops, s, TYPE_MACH_KIRKWOOD, 0x200);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1090000, &s->stub39_mmio, 0);
    memory_region_init_io(&s->stub40_mmio, NULL, &stub40_ops, s, TYPE_MACH_KIRKWOOD, 0x2210);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf10a0000, &s->stub40_mmio, 0);
    memory_region_init_io(&s->stub43_mmio, NULL, &stub43_ops, s, TYPE_MACH_KIRKWOOD, 0x100000);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf2000000, &s->stub43_mmio, 0);
    memory_region_init_io(&s->stub41_mmio, NULL, &stub41_ops, s, TYPE_MACH_KIRKWOOD, 0x400);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf4000000, &s->stub41_mmio, 0);
    memory_region_init_io(&s->stub42_mmio, NULL, &stub42_ops, s, TYPE_MACH_KIRKWOOD, 0x800);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf5000000, &s->stub42_mmio, 0);
    memory_region_init_io(&s->stub45_mmio, NULL, &stub45_ops, s, TYPE_MACH_KIRKWOOD, 0x1000);
    memory_region_add_subregion_overlap(get_system_memory(), 0xf1044000, &s->stub45_mmio, 0);
    
    mach_kirkwood_reset(s);
...
static void mach_kirkwood_machine_init(MachineClass *mc)
{
    mc->desc = "mach_kirkwood";
    mc->init = mach_kirkwood_init;
    mc->default_ram_size = 0x8000000;
    mc->default_cpu_type = ARM_CPU_TYPE_NAME("feroceon");
    mc->ignore_memory_transaction_failures = false;
}

DEFINE_MACHINE("mach_kirkwood", mach_kirkwood_machine_init)
